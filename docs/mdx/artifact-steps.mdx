...
**Technical details:** `classify_ica_components` calls AutoClean's classifier wrappers (ICLabel, ICVision, or a hybrid of both). In hybrid mode AutoClean runs ICLabel across all components then rechecks leading components with ICVision. Components revisited by ICVision are marked with `annotator="ic_vision"` and appear as `[Vision]` in reports. `apply_ica_component_rejection` then drops components exceeding an optional probability threshold.
