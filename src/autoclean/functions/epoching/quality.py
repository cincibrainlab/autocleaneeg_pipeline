    ...
    **Threshold Selection:**
    ...
    try:
        ...
        if 'gradient' in measures:
            ...
            z_scores = np.abs((mean_gradient - np.mean(mean_gradient)) / np.std(mean_gradient))
        ...
        if outlier_list:
            ...
            if verbose:
                print(f"Marked {len(outlier_list)} epochs as outliers (threshold={threshold})")
        elif verbose:
    ...
    **Random Subsampling:**
    ...
    try:
        ...
        if len(outlier_indices) > 0:
            ...
            if verbose:
                print(f"Removed {len(outlier_indices)} epochs based on GFP outlier detection")
        elif verbose:
        ...
        if number_of_epochs is not None:
            ...
            if verbose:
                print(f"Randomly selected {number_of_epochs} epochs from {remaining_epochs} available")
