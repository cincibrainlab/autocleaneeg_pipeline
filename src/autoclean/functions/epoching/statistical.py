...
def create_sl_epochs(
    ...
    **Epoch Timing:**
    ...
    if num_syllables_per_epoch <= 0:
        raise ValueError(f"num_syllables_per_epoch must be positive, got {num_syllables_per_epoch}")
    ...
    try:
        ...
        if data_copy.annotations is not None:
            di64_indices = [
                i for i, desc in enumerate(data_copy.annotations.description)
                if desc == "DI64"
            ]
            if di64_indices:
        ...
        try:
            events_all, event_id_all = mne.events_from_annotations(data_copy, verbose=verbose)
        ...
        if not word_onset_ids:
            raise ValueError(f"No word onset events found. Expected: {word_onset_codes}, Available: {list(event_id_all.keys())}")
        ...
        if not syllable_code_ids:
            raise ValueError(f"No syllable events found. Expected: {syllable_codes}, Available: {list(event_id_all.keys())}")
        ...
        if "No events found" in str(e) or "No valid epochs" in str(e):
            ...
            raise
        raise RuntimeError(f"Failed to create statistical learning epochs: {str(e)}") from e


def _add_sl_metadata(epochs: mne.Epochs, raw: mne.io.BaseRaw, events_all: np.ndarray, event_id_all: Dict) -> mne.Epochs:
