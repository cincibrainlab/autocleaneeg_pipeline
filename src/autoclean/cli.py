...
Examples:
  ...
  autoclean config show
    ...
    process_parser.add_argument(
        '--file', type=Path, help='Single EEG file to process (alternative to positional)'
    )
    process_parser.add_argument(
        '--dir', '--directory', type=Path, dest='directory', 
    ...
    add_task_parser.add_argument(
        'task_file', type=Path, help='Python task file to add'
    )
    add_task_parser.add_argument(
    ...
    # Remove task
    remove_task_parser = task_subparsers.add_parser('remove', help='Remove a custom task')
    ...
    try:
        ...
        if args.task_file:
            ...
            if custom_task_path:
                ...
                message("info", f"Loaded custom task '{args.final_task}' from user configuration")
...
def cmd_list_tasks(args) -> int:
    ...
    try:
        ...
        if args.include_custom:
            ...
            if custom_tasks:
                print()
                print(f"  ({len(custom_tasks)} custom tasks available - use --include-custom to show)")
...
def cmd_task_add(args) -> int:
    """Add a custom task to user configuration."""
    try:
        ...
        task_name = args.name or args.task_file.stem
        existing_tasks = user_config.list_custom_tasks()
        
        if task_name in existing_tasks and not args.force:
            message("error", f"Task '{task_name}' already exists. Use --force to overwrite.")
            return 1
        
        # Add the task
        saved_name = user_config.add_custom_task(args.task_file, args.name)
        message("info", f"Custom task '{saved_name}' added successfully!")
        
        # Show usage
        print(f"\nUse your custom task with:")
        print(f"  autoclean process --task {saved_name} --file <data_file>")
...
def cmd_task_remove(args) -> int:
    """Remove a custom task from user configuration."""
    try:
        if user_config.remove_custom_task(args.task_name):
            message("info", f"Custom task '{args.task_name}' removed successfully!")
            return 0
        else:
            message("error", f"Custom task '{args.task_name}' not found")
            return 1
