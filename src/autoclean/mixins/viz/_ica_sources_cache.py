...
class ICASourcesCache:
    ...
    def _cleanup_if_needed(self, required_size: int):
        ...
        for key, _ in sorted_keys:
            if key in self._cache:
                ...
                logger.debug(
                    f"Removed cache entry {key}, freed {removed_size/1024/1024:.1f}MB"
                )
    ...
    def get_sources(
        ...
        logger.debug(
            f"Cached sources: {data_size/1024/1024:.1f}MB, "
    ...
    def invalidate_ica(self, ica: ICA):
        ...
        if keys_to_remove:
            logger.debug(
                f"Invalidated {len(keys_to_remove)} cache entries for ICA {ica_id}"
            )
