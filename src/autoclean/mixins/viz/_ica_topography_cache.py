...
class ICATopographyCache:
    ...
    def __init__(self, max_cache_size_mb: float = 200.0):
        ...
        logger.debug(
            f"ICA topography cache initialized with {max_cache_size_mb}MB limit"
        )
    ...
    def get_topographies(
        ...
        with self._lock:
            ...
            if not force_refresh and cache_key in self._cache:
                ...
                logger.debug(
                    f"Cache partial miss - have {len(result)}/{len(component_indices)} components"
                )
            ...
            logger.debug(
                f"Batch topographies computed in {computation_time:.2f}s for {len(component_indices)} components"
            )
        ...
        try:
            ...
            for idx in component_indices:
                try:
                    ...
                    logger.warning(
                        f"Failed to compute topography for component {idx}: {exc}"
                    )
