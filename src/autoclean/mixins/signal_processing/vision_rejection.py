...
try:
    ...
    message(
        "warning",
        "python-dotenv not installed. Cannot load .env file. Ensure OPENAI_API_KEY is set manually or via environment.",
    )
...
"""
    ...
    def annotate_bad_segments_vision(
        ...
        if raw is None:
            raise ValueError(
                "Raw data is not provided and cannot be loaded from the instance."
            )

        # Check if raw is an instance of mne.io.Raw or any of its subclasses
        if not isinstance(raw, mne.io.BaseRaw):
            raise TypeError(
                "Input must be an mne.io.Raw object or a subclass of mne.io.BaseRaw."
            )
        if not raw.preload:
            message(
                "warning",
                "Raw data is not preloaded. Loading data now for vision analysis.",
            )
            try:
                raw.load_data()
        ...
        message(
            "header",
            f"Starting vision-based bad segment annotation (Window: {window_duration}s, Threshold: {confidence_threshold})...",
        )
        ...
        try:
            ...
            if len(temp_epochs) == 0:
                message(
                    "warning",
                    "No segments (epochs) could be created from the raw data.",
                )
                return mne.Annotations([], [], [], orig_time=raw.info.get("meas_date"))
            ...
            num_to_process = (
                ...
                else min(len(temp_epochs), max_segments)
            )
            message(
                "info",
                f"Created {len(temp_epochs)} temporary segments. Processing up to {num_to_process}.",
            )

        except Exception as e:
            message(
                "error",
                f"Failed to create temporary fixed-length epochs from Raw data: {e}",
            )
            return mne.Annotations(
                [], [], [], orig_time=raw.info.get("meas_date")
            )  # Return empty annotations on failure
        ...
        with tempfile.TemporaryDirectory(prefix="autoclean_vision_") as temp_dir:
            ...
            for i in range(num_to_process):
                segment_start_time = (
                    temp_epochs.events[i, 0] - raw.first_samp
                ) / raw.info[
                    "sfreq"
                ]  # Start time in seconds from raw beginning
                ...
                if (
                    ...
                    for annot in raw.annotations:
                        ...
                        if annot["description"].lower().startswith("bad"):
                            ...
                            if (segment_start_time < annot_end) and (
                                segment_end_time > annot_start
                            ):
                                message(
                                    "debug",
                                    f"Segment {i+1} ({segment_start_time:.2f}s) overlaps with existing annotation '{annot['description']}' ({annot_start:.2f}s). Skipping.",
                                )
                                overlaps = True
                ...
                try:
                    ...
                    message(
                        "warning",
                        f"Failed to plot segment {i+1}: {plot_err}. Skipping classification for this segment.",
                    )
                    continue  # Skip classification if plotting fails

                # --- Classify the image ---
                try:
                    label, confidence, reason = self._classify_epoch_image_openai(
                        image_path, api_key
                    )
                except (
                    Exception
                ) as classify_err:  # Catch any unexpected error during classification call itself
                    message(
                        "warning",
                        f"Classification failed for segment {i+1} ({image_path.name}): {classify_err}. Defaulting to 'bad'.",
                    )
                    label, confidence, reason = (
                        "bad",
                        1.0,
                        f"Classification call failed: {classify_err}",
                    )
                ...
                if label == "good":
                    ...
                    if confidence >= confidence_threshold:
                        log_level = (
                            "warning"  # Use WARNING for bad segments meeting threshold
                        )
        ...
        if onsets:  # Only create if we found bad segments
            bad_annotations = mne.Annotations(
                ...
                orig_time=(
                    ...
                    if raw.annotations
                    else raw.info.get("meas_date")
                ),  # Preserve original time reference
            )
            message(
                "info",
                f"Completed vision scan. Found {len(bad_annotations)} segments to annotate as '{annotation_label}'.",
            )
        else:
            bad_annotations = mne.Annotations(
                [], [], [], orig_time=raw.info.get("meas_date")
            ...
            message(
                "info",
                "Completed vision scan. No segments met criteria for annotation.",
            )
        ...
        if (
            hasattr(self, "config")
            and self.config
            and "derivatives_dir" in self.config
            and "bids_path" in self.config
        ):
            ...
            message(
                "warning", "Configuration not found. Skipping PDF report generation."
            )
    ...
    def find_bad_epochs_vision(
        ...
        if not epochs.preload:
            message(
                "warning",
                "Epochs data is not preloaded. Loading data now for vision analysis.",
            )
            try:
                epochs.load_data()
            except Exception as e:
        ...
        message(
            "header",
            f"Starting vision-based bad epoch identification (Threshold: {confidence_threshold})...",
        )
        ...
        processed_count = 0
        num_to_process = (
            len(epochs) if max_epochs is None else min(len(epochs), max_epochs)
        )
        ...
        with tempfile.TemporaryDirectory(prefix="autoclean_vision_") as temp_dir:
            ...
            for i in range(num_to_process):
                ...
                try:
                    ...
                    message(
                        "warning", f"Failed to plot epoch {i}: {plot_err}. Skipping."
                    )
                    continue
                ...
                try:
                    ...
                    Exception
                ) as classify_err:  # Catch unexpected error during classification call
                    message(
                        "warning",
                        f"Classification failed for epoch {i} ({image_path.name}): {classify_err}. Defaulting to 'bad'.",
                    )
                ...
                log_level = "debug"
                is_bad_above_threshold = (
                    False  # We'll re-check this in the metadata loop
                )
                if label == "good":
                    log_level = "success"
                elif label == "bad":
        ...
        message(
            "info",
            f"Completed vision scan. Processed {processed_count} epochs. Updating metadata...",
        )

        # --- Update Epochs Metadata ---
        if not classification_results:
            message(
                "info", "No epochs were classified. Returning original epochs object."
            )
            return epochs
        ...
        if epochs.metadata is None:
            message(
                "info", "Epochs metadata not found. Creating new metadata DataFrame."
            )
        ...
        for result in classification_results:
            ...
            if is_bad_above_threshold:
                ...
                if idx in epochs.metadata.index:
                    ...
                    message(
                        "warning",
                        f"Epoch index {idx} not found in metadata. Cannot mark as bad.",
                    )

        message(
            "info",
            f"Marked {bad_epoch_count} epochs as '{metadata_label}' in metadata.",
        )
    ...
    def _plot_single_epoch_for_vision(
        self,
        epoch_data: np.ndarray,
        times: np.ndarray,
        segment_identifier: Union[int, str],
        output_dir: Path,
    ) -> Path:
        """
        ...
        if epoch_data.ndim != 2:
            raise ValueError(
                f"Expected 2D epoch_data (channels, times), got shape {epoch_data.shape}"
            )
        ...
        if n_channels == 0 or n_times == 0:
            message(
                "warning",
                f"Skipping plot for segment {segment_identifier} due to empty data ({n_channels}x{n_times}).",
            )
            return None  # Indicate plotting was skipped

        message(
            "debug",
            f"Plotting segment {segment_identifier} with {n_channels} channels, {n_times} timepoints.",
        )
        ...
        max_abs_val = (
            0  # Track max absolute value for potential scaling adjustment if needed
        )
        ...
        for idx, channel_data in enumerate(data_uv):
            ...
            max_abs_val = max(
                max_abs_val, np.max(np.abs(channel_data))
            )  # Track original data max amplitude
        ...
        channel_labels = [
            str(idx + 1) for idx in range(n_channels)
    ...
    def _classify_epoch_image_openai(
        self, image_path: Path, api_key: Optional[str] = None
    ) -> Tuple[str, float, str]:
        """
        ...
        if not image_path or not image_path.exists():
            message(
                "error",
                "Invalid or non-existent image path provided for classification.",
            )
        ...
        try:
            ...
            if not effective_api_key:
                ...
                if hasattr(openai, "api_key") and openai.api_key:
                    ...
                    raise ValueError(
                        "OpenAI API key not provided via argument, environment variable (OPENAI_API_KEY), or openai.api_key."
                    )
                ...
                input=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "input_text", "text": self._OPENAI_PROMPT},
                            {
                                "type": "input_image",
            ...
            if resp_text is not None:
                ...
                match = re.search(
                    ...
                    resp_text,
                    re.IGNORECASE
                    | re.DOTALL,  # Ignore case, allow '.' to match newline
                )
                if match:
                    ...
                    reason = (
                        ...
                        .replace("\\\\'", "'")
                        .replace('\\\\"', '"')
                    )  # Clean up captured reason
                    # Changed level to DEBUG
                    message(
                        "debug",
                        f"Parsed classification: Label={label}, Conf={confidence:.2f}, Reason='{reason[:50]}...'",
                    )
                    return label, confidence, reason
                else:
                    message(
                        "warning",
                        f"Could not parse OpenAI response format: '{resp_text}'. Defaulting to 'bad'.",
                    )
                ...
                message(
                    "error",
                    "Could not extract text content from OpenAI response structure.",
                )
            ...
            message(
                "error", f"OpenAI API authentication error: {e}. Check your API key."
            )
            ...
            message(
                "error",
                f"OpenAI API status error: Status={e.status_code}, Response={e.response}",
            )
            ...
            message(
                "error",
                f"Unexpected exception during vision classification of {image_path.name}: {type(e).__name__} - {e}",
            )
    ...
    def _generate_vision_segments_report_pdf(
        ...
        try:
            ...
            message(
                "error", f"Could not create output directory {output_dir}: {e_mkdir}"
            )
            return None
        ...
        if pdf_path.exists():
            try:
                ...
                message(
                    "warning", f"Could not delete existing PDF {pdf_path}: {e_unlink}"
                )
        ...
        bad_segments = [
            ...
            if r["label"] == "bad" and r["confidence"] >= 0.8
        ]
        ...
        message(
            "info", f"Generating PDF report for {len(bad_segments)} bad segments..."
        )
            ...
            with PdfPages(pdf_path) as pdf:
                ...
                for segment in bad_segments:
                    ...
                    end_sample = start_sample + int(
                        window_duration * raw_obj.info["sfreq"]
                    )
                    ...
                    times_segment = (
                        np.arange(data_segment.shape[1]) / raw_obj.info["sfreq"]
                    )

                    if data_segment.ndim == 1:
                        data_segment = data_segment[
                            np.newaxis, :
                        ]  # Ensure 2D for plotting multiple channels (even if 1)

                    for ch_data in data_segment:
                        ax_data.plot(times_segment, ch_data.T)  # Plot each channel

                    ax_data.set_title(
                        f"Segment {segment['index'] + 1} - Bad Segment Visualization"
                    )
                    ...
                    fig_detail.tight_layout(
                        rect=[0, 0, 1, 0.96]
                    )  # Adjust rect to make space for suptitle if any, or just use tight_layout()
