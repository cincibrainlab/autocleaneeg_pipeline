...
def estimate_source_function_epochs(epochs: mne.Epochs, config: dict = None):
    ...
    stc = mne.minimum_norm.apply_inverse_epochs(
        epochs, inv, lambda2=1./9., method='MNE', pick_ori='normal', verbose=True
    )

    print("Computed source estimates for epochs using MNE with identity noise covariance")
    ...
    if config is not None:
        ...
        if isinstance(stc, list):
            ...
            for i, stc_epoch in enumerate(stc[:min(3, len(stc))]):
                save_stc_to_file(stc_epoch, config, stage=f"post_source_localization_epoch_{i}")
        else:
            save_stc_to_file(stc, config, stage="post_source_localization")
...
def calculate_source_psd_list(stc_list, subjects_dir=None, subject='fsaverage', n_jobs=4, 
                             output_dir=None, subject_id=None, generate_plots=True,
                             segment_duration=80):
    """
    ...
    print(f"Total available signal: {total_duration:.1f} seconds ({len(stc_list)} epochs of {epoch_duration:.1f}s each)")
    ...
    if segment_duration is not None and segment_duration < total_duration:
        ...
        print(f"Using {len(selected_stcs)} epochs ({actual_duration:.1f}s) from the middle of the recording")
    else:
    ...
    print(f"Processing {n_vertices} vertices in {n_batches} batches of size {batch_size}...")
    
    # Create batches
    batch_indices = [range(i * batch_size, min((i + 1) * batch_size, n_vertices)) 
    ...
    if generate_plots and all_viz_vertices:
        ...
        def create_psd_plot(vertex_idx, f_pxx):
            ...
            plt.xlim([0, 50])
            plt.grid(True)
            plt.savefig(os.path.join(output_dir, "psd_plots", f"{subject_id}_vertex{vertex_idx}_psd.png"))
            plt.close()
    ...
    def process_label(label_idx):
        ...
        if label.hemi == 'lh':
            ...
            # Right hemisphere
            stc_idx = np.where(np.in1d(selected_stcs[0].vertices[1], label_verts))[0] + len(selected_stcs[0].vertices[0])
    ...
    if generate_plots:
        ...
        band_order = ['delta', 'theta', 'lowalpha', 'highalpha', 'alpha', 'lowbeta', 'highbeta', 'gamma']
...
def calculate_source_connectivity(stc, labels=None, subjects_dir=None, subject='fsaverage', 
    ...
    logger.info(f"Calculating connectivity for {subject_id} with {n_epochs} {epoch_length}-second epochs (sfreq={sfreq} Hz)...")
    ...
    if max_epochs < n_epochs:
        logger.warning(f"Requested {n_epochs} epochs, but only {max_epochs} possible. Using {max_epochs}.")
        n_epochs = max_epochs
    ...
    for method in conn_methods:
        for band_name, band_range in bands.items():
            ...
            try:
                ...
                logger.info(f"  Parameters: freqs={np.arange(band_range[0], band_range[1] + 1)}, "
                ...
                logger.info(f"{method} {band_name} con_matrix shape: {con_matrix.shape}")
                logger.debug(f"Matrix values range: min={np.min(con_matrix)}, max={np.max(con_matrix)}")
                ...
                logger.error(f"Data shape: {epoched_data.shape}, freqs: {np.arange(band_range[0], band_range[1] + 1)}")
                continue
    ...
    for method in list(conn_methods) + (['aec'] if include_aec else []):
        for band_name in bands.keys():
            ...
            if subset_df.empty:
                logger.warning(f"No data for {method} in {band_name} band. Skipping graph metrics.")
                continue
            ...
            if np.isnan(adj_matrix).any():
                logger.warning(f"NaN values in {method} {band_name} adjacency matrix. Skipping graph metrics.")
                continue
                
            # For some metrics like clustering coefficient, we need positive weights
            if (adj_matrix < 0).any():
                logger.warning(f"Negative values in {method} {band_name} matrix. Using absolute values for graph metrics.")
            ...
            try:
                ...
                small_worldness = clustering * char_path_length if char_path_length > 0 else 0
                ...
                logger.info(f"Calculated graph metrics for {method} in {band_name} band")
            except Exception as e:
...
def test_connectivity_function_list():
    ...
    for epoch in range(n_epochs):
        ...
        stc = SourceEstimate(source_data.copy(), vertices, tmin=0, tstep=1/sfreq, subject='simulated')
        stc_list.append(stc)
    ...
    try:
        ...
        for i, name in enumerate(label_names):
            ...
            label = mne.Label(
                vertices, pos, hemi=hemi,
                name=name, subject='simulated'
            )
            labels.append(label)
            ...
            if not conn_df.empty:
                ...
                for method in conn_df['method'].unique():
                    for band in conn_df['band'].unique():
                        subset = conn_df[(conn_df['method'] == method) & (conn_df['band'] == band)]
                        if not subset.empty:
                            avg_conn = subset['connectivity'].mean()
                            print(f"  {method} ({band}): Mean connectivity = {avg_conn:.4f}")
...
def calculate_source_connectivity_list(stc_list, labels=None, subjects_dir=None, subject='fsaverage', 
                                 n_jobs=4, output_dir=None, subject_id=None, sfreq=None,
                                 epoch_length=4.0, n_epochs=20, max_duration=80):
    """
    Calculate connectivity metrics between brain regions from source-localized data.
    ...
    logger.info(f"Total available data: {total_duration:.1f} seconds ({len(stc_list)} epochs of {single_epoch_duration:.1f}s each)")
    ...
    if max_duration and total_duration > max_duration:
        ...
        logger.info(f"Limited to {actual_duration:.1f} seconds ({len(stc_list)} epochs)")
    
    logger.info(f"Calculating connectivity for {subject_id} with {n_epochs} {epoch_length}-second epochs (sfreq={sfreq} Hz)...")
    
    #bands = {'delta': (1, 4), 'theta': (4, 8), 'alpha': (8, 13), 'beta': (13, 30), 'gamma': (30, 45)}
    bands = {'theta': (4, 8), 'lowalpha': (8, 10), 'highalpha': (10, 13), 'beta': (13, 30), 'gamma': (30, 45)}
    ...
    epoched_data = np.stack([roi_data[:, start:start + samples_per_epoch] 
                            for start in epoch_starts if start + samples_per_epoch <= n_times], axis=0)
    logger.info(f"Epoched data shape: {epoched_data.shape}")
    ...
    total_time = time.time() - start_time
    logger.info(f"Connectivity analysis complete for {subject_id} in {total_time:.1f} seconds ({total_time/60:.1f} minutes)")
    logger.info(f"Log file saved to: {log_file}")
...
def test_connectivity_function():
    ...
    stc = SourceEstimate(source_data, vertices, tmin=0, tstep=1/sfreq, subject='simulated')
...
def calculate_aec_connectivity(stc_list, labels=None, subjects_dir=None, subject='fsaverage',
                            sfreq=None, output_dir=None, subject_id=None, 
                            epoch_length=2.0, n_epochs=40):
    """
    ...
    for band_name, band_range in bands.items():
        ...
        matrix_filename = os.path.join(output_dir, f"{subject_id}_aec_{band_name}_matrix.csv")
        con_df.to_csv(matrix_filename)
        logger.info(f"Saved connectivity matrix to {matrix_filename}")
...
def calculate_vertex_level_spectral_power_list(stc_list, bands=None, n_jobs=10, output_dir=None, subject_id=None):
    """
    ...
    for vertex_idx in tqdm(sample_indices, desc="Sampling vertex variance"):
        try:
            ...
            logging.error(f"Error calculating variance for vertex {vertex_idx}: {str(e)}")
    ...
    if len(non_zero_vars) > 0:
        var_threshold = np.percentile(non_zero_vars, 10)  # 10th percentile of non-zero values
    else:
    ...
    plt.axvline(np.log10(var_threshold), color='r', linestyle='--', 
        ...
        for i, vertex_idx in enumerate(vertex_indices):
            try:
                ...
                f, psd = signal.welch(
                    ...
                    fs=sfreq, 
                    window='hann',
                    nperseg=min(len(vertex_data), int(4 * sfreq)),  # 4-second windows or shorter
                ...
                if vertex_idx % 2000 == 0:
                    ...
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(os.path.join(output_dir, "psd_plots", 
                ...
                for band, (fmin, fmax) in bands.items():
                    ...
                    if np.any(freq_mask):
                        ...
                        logging.warning(f"No frequencies in band {band} ({fmin}-{fmax} Hz) for vertex {vertex_idx}")
    ...
    batch_size = 1000
    n_batches = int(np.ceil(n_vertices / batch_size))
    all_vertex_batches = [range(i * batch_size, min((i + 1) * batch_size, n_vertices)) for i in range(n_batches)]
    ...
    band_order = sorted(bands.keys(), 
                        key=lambda x: bands[x][0])  # Sort by lower bound of band
    
    # Create bar plot
    plt.bar(range(len(band_order)), 
    ...
    logging.info(f"Generated PSD plots for sample vertices in {os.path.join(output_dir, 'psd_plots')}")
