...
def compute_time_frequency(epochs, freqs=None, n_cycles=None, baseline=(-0.5, 0)):
    ...
    if n_cycles is None:
        n_cycles_base = freqs / 2.  # Base cycles
...
def compute_metrics(tf_data, epochs, freq_bands=None, time_windows=None):
    ...
    n_rejected_trials = sum(1 for log in epochs.drop_log if log)  # Count non-empty drop logs
    ...
    for ch_idx, ch_name in enumerate(epochs.ch_names):
        ...
        time_all_idx = find_time_indices(itc[1].times, *time_windows['all'])  # itc[1] for actual ITC values
        ...
        itc_onset = np.mean(itc[1].data[ch_idx, itc_onset_freq_idx][:, itc_onset_time_idx])
        
        # Calculate ITC offset (2-13 Hz, 2800-3000 ms)
        itc_offset_freq_idx = find_freq_indices(freqs, *freq_bands['itc_onset'])  # Same frequency band as onset
        itc_offset_time_idx = find_time_indices(itc[1].times, *time_windows['itc_offset'])
        itc_offset = np.mean(itc[1].data[ch_idx, itc_offset_freq_idx][:, itc_offset_time_idx])
...
def analyze_assr(file_path=None, output_dir=None, save_results=True, epochs=None, file_basename=None):
    ...
    print(f"Maximum ITC value at 40 Hz: {np.max(tf_data['itc'][1].data[:, freq_idx, :]):.3f}")
    
    # Find midrange frequencies around 40 Hz to compute resolution
    freqs_mid = [f for f in tf_data['freqs'] if 30 <= f <= 50]
    if len(freqs_mid) > 1:
        print(f"Frequency resolution around 40 Hz: {freqs_mid[1] - freqs_mid[0]:.2f} Hz")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Analyze ASSR data from EEGLAB .set files')
