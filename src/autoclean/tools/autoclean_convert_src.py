...
from autoclean.calc.source import (
    estimate_source_function_raw,  # Import the source function
)
...
# Define directories
INPUT_DIR = Path(
    "/Users/ernie/Data/autoclean_v3/resting_eyesopen_grael4k/stage/04_postrejection"
)
OUTPUT_DIR = Path(
    "/Users/ernie/Data/autoclean_v3/resting_eyesopen_grael4k/stage/09_stc"
)
CONFIG = {
    "stage_dir": OUTPUT_DIR,
    "stage_files": {"post_source_localization": {"enabled": True, "suffix": "_stc"}},
...
def process_and_save_stc(
    raw: mne.io.Raw, output_dir: Path, config: dict, filename: str
):
    ...
    try:
        ...
        console.print(
            f"[bold cyan]Computing source estimates for {filename}[/bold cyan]"
        )
        stc = estimate_source_function_raw(raw)  # Assuming this returns an STC object
        console.print(
            f"[bold green]Successfully computed STC with shape: {stc.data.shape} (vertices x times)[/bold green]"
        )
...
def main():
    ...
    console.print(
        f"[bold blue]Found {len(set_files)} .set files to process[/bold blue]"
    )
