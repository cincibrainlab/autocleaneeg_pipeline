"""AutoClean Exclude GUI.

Manual epoch review and exclusion workflow for AutoClean EEG runs. Install the
GUI extras with ``pip install autocleaneeg-pipeline[gui]`` before launching the
tool.

The implementation now relies on :mod:`qtpy` so the application can run on top
of either PyQt5 or PyQt6 without code changes.
"""
...
except ImportError as exc:  # pragma: no cover - handled in dependency check
    raise ImportError(
        "qtpy is required for the autoclean_exclude GUI. Install the 'gui' extra"
        " via 'pip install autocleaneeg-pipeline[gui]'."
    ) from exc
...
def check_gui_dependencies() -> None:
    ...
    if missing:
        ...
        print("To use the exclude tool, install the GUI dependencies:")
...
def run_autoclean_exclude(autoclean_dir):
...
if __name__ == "__main__":
    run_autoclean_exclude(Path("C:/Users/Gam9LG/Documents/Autoclean"))
