    ...
    def import_and_configure(self, file_path: Path, autoclean_dict: dict, preload: bool = True):
        ...
        try:
            ...
            if len(raw.ch_names) != 30:
                message("warning", f"Expected 30 channels for MEA30, found {len(raw.ch_names)}")
            ...
            raise RuntimeError(f"Failed to process EEGLAB file with MEA30 montage: {str(e)}")
        ...
        try:
            ...
            if events is not None and len(events) > 0:
                events_df = pd.DataFrame({
                    ...
                    'id': events[:, 2],
                    'type': [event_id.get(id, f"Unknown-{id}") for id in events[:, 2]]
                })
