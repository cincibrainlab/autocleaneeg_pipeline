...
"""BioSemi BDF plugin for biosemi32 montage.

This module provides an EEG plugin for handling BioSemi BDF files
with the biosemi32 electrode montage. It follows the plugin and mixin
...
class BiosemiBDFPlugin(BDFImportMixin, BaseEEGPlugin):
    ...
    def supports_format_montage(cls, format_id: str, montage_name: str) -> bool:
        ...
        return format_id == "BIOSEMI_BDF" and montage_name.lower() == "biosemi32"

    def import_and_configure(
        self, file_path: Path, autoclean_dict: dict, preload: bool = True
    ) -> mne.io.Raw:
        """Import BDF data and apply biosemi32 montage."""
        message("info", f"Loading BDF file with BioSemi32 montage: {file_path}")
