...
class RestingStateEventProcessor(BaseEventProcessor):
    ...
    def process_events(self, 
        ...
        if not self._check_config_enabled("resting_state_event_processing", autoclean_dict):
            message("info", "✗ Resting state event processing disabled in configuration")
            return raw
        ...
        condition = autoclean_dict.get("resting_state_condition", 
                                     autoclean_dict.get("condition", ""))
                                     
        # Get description format from config or use default
        description_format = autoclean_dict.get("resting_state_description_format",
                                              "resting_state/{condition}")
        
        if condition and (condition.lower() == "eyes_open" or condition.lower() == "eyes_closed"):
            try:
                ...
                new_annotation = mne.Annotations(
                    ...
                    description=[description_format.format(condition=condition.lower())]
                )
                ...
                message("success", f"Added resting state annotation with condition: {condition}")
        ...
        if self._check_config_enabled("resting_state_create_segments", autoclean_dict, False):
            # Calculate total duration
            duration = raw.times[-1]
            
            # Get segment duration from config or use default
            segment_duration = autoclean_dict.get("resting_state_segment_duration", 5.0)  # seconds
            
            # Get segment description from config or use default
            segment_description = autoclean_dict.get("resting_state_segment_description", "segment")
            ...
            try:
                ...
                message("success", f"Created {len(segment_times)} segment annotations for resting state data")
            except Exception as e:
