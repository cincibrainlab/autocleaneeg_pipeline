...
def load_config(config_file: Path) -> dict:
    ...
    config_schema = Schema(
        {
            "tasks": {
                str: {
                    ...
                    "lossless_config": str,
                    "settings": {
                        "resample_step": {"enabled": bool, "value": Or(int, float, None)},
    ...
    for task in autoclean_dict["tasks"]:
        autoclean_dict = validate_pylossless_config(autoclean_dict, task, config_file_path)
        validate_signal_processing_params(autoclean_dict, task)
    
    return autoclean_dict

def validate_pylossless_config(autoclean_dict: dict, task: str, config_file_path: Path) -> None:
    ...
    message("debug", f"[{task}] Validating pylossless config at path: {lossless_config_path}")
    ...
    if not Path(lossless_config_path).exists():
        ...
        message("debug", f"[{task}] Trying to find pylossless config relative to main config file directory")
        ...
        if relative_path.exists():
            ...
            message("error", f"[{task}] Could not find pylossless config at {lossless_config_path} or {relative_path}")
...
def validate_signal_processing_params(autoclean_dict: dict, task: str) -> None:
    ...
    try:
        ...
        message('error', f'Failed to validate config ({lossless_config_path}) for task: {task}')
        raise e
    ...
    if any(freq >= nyquist_freq for freq in h_freq):
        message("error", f"High-pass filter frequency {h_freq} must be below Nyquist frequency {nyquist_freq} Hz")
        raise ValueError(f"Filter frequency {h_freq} exceeds Nyquist frequency {nyquist_freq} Hz")
    ...
    if any(freq >= nyquist_freq for freq in l_freq):
        message("error", f"Low-pass filter frequency {l_freq} must be below Nyquist frequency {nyquist_freq} Hz")
        raise ValueError(f"Filter frequency {l_freq} exceeds Nyquist frequency {nyquist_freq} Hz")
    ...
    if epoch_settings["enabled"]:
        ...
        if tmin is not None and tmax is not None:
            if tmax <= tmin:
                message("error", f"Epoch tmax ({tmax}s) must be greater than tmin ({tmin}s)")
