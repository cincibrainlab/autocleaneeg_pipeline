...
class HBCD_MMN(Task):
    ...
    def _verify_annotations(self, raw, stage_name=""):
        ...
        if stage_name == "post_prepipeline":
            trim_settings = self.config["tasks"][self.config["task"]]["settings"]["trim_step"]
            if trim_settings["enabled"]:
                self._trim_amount = trim_settings["value"]
                message("info", f"Updated trim amount to {self._trim_amount}s from config")
        ...
        if len(current_onsets) != len(adjusted_onsets):
            ...
            message("warning", f"Original count (after trim adjustment): {len(adjusted_onsets)}")
            message("warning", f"Current count: {len(current_onsets)}")
            return False
            
        try:
            np.testing.assert_array_almost_equal(current_onsets, adjusted_onsets, decimal=6)
            message("info", f"✓ DIN2 event timing verified at {stage_name}")
            return True
        except AssertionError:
            # Find first mismatch for detailed analysis
            mismatch_idx = np.where(np.abs(current_onsets - adjusted_onsets) > 1e-6)[0][0]
            message("warning", f"✗ DIN2 event timing changed at {stage_name}:")
            message("warning", f"First mismatch at event {mismatch_idx}:")
            message("warning", f"Original onset time (adjusted for trim): {adjusted_onsets[mismatch_idx]:.6f}")
            message("warning", f"Current onset time: {current_onsets[mismatch_idx]:.6f}")
            message("warning", f"Difference: {current_onsets[mismatch_idx] - adjusted_onsets[mismatch_idx]:.6f}s")
    ...
    def preprocess(self) -> None:
        ...
        self.cleaned_raw = step_detect_dense_oscillatory_artifacts(self.cleaned_raw, window_size_ms=100, channel_threshold_uv=50, min_channels=75, padding_ms=500)
