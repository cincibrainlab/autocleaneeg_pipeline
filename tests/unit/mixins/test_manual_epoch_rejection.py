...
class TestManualEpochRejection:
    ...
    def test_drops_manual_epoch_positions_after_existing_epoch_drops(self, task):
        with (
            patch.object(task, "_update_metadata") as mock_update,
            patch.object(task, "_auto_export_if_enabled"),
        ):
            result = task.drop_manual_bad_epochs(
                manual_bad_epoch_positions=[1, 2, 99],
            )
        ...
        metadata = mock_update.call_args.args[1]
        assert metadata["requested_bad_epoch_positions"] == [1, 2, 99]
        assert metadata["applied_bad_epoch_positions"] == [1, 2]
