        ...
        "tasks": {
            "test_task": {
                ...
                "settings": {
                    ...
                    "eog_step": {"enabled": True, "value": raw.ch_names[:3]},  # Use first 3 channels for EOG
...
def test_pipeline_initialization(tmp_path, test_config):
    ...
    # Initialize pipeline
    pipeline = Pipeline(
        autoclean_dir=str(tmp_path),
        autoclean_config=str(config_path)
    )
