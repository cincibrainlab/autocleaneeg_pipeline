...
def create_test_montage_info(montage: str, n_channels: int, sfreq: float) -> mne.Info:
    ...
    if 'GSN-HydroCel' in montage:
        ...
        standard_names = ['Fp1', 'Fp2', 'F7', 'F3', 'Fz', 'F4', 'F8', 'FC5', 'FC1', 'FC2', 'FC6',
    ...
    if len(ch_names) != n_channels:
        ch_names = ch_names[:n_channels] if len(ch_names) > n_channels else ch_names + [f'EEG{i:03d}' for i in range(len(ch_names), n_channels)]
...
def create_synthetic_eeg_signal(n_channels: int, n_samples: int, sfreq: float, 
    ...
    for ch in range(n_channels):
        ...
        alpha = alpha_power * np.sin(2 * np.pi * alpha_freq * times + np.random.uniform(0, 2*np.pi))
        
        # Add beta rhythm (13-30 Hz)
        beta_power = 5e-6
        beta_freq = np.random.uniform(15, 25)
        beta = beta_power * np.sin(2 * np.pi * beta_freq * times + np.random.uniform(0, 2*np.pi))
        ...
        theta = theta_power * np.sin(2 * np.pi * theta_freq * times + np.random.uniform(0, 2*np.pi))
        
        # Add slow waves (1-4 Hz)
        slow_power = 8e-6
        slow_freq = np.random.uniform(1, 4)
        slow = slow_power * np.sin(2 * np.pi * slow_freq * times + np.random.uniform(0, 2*np.pi))
        
        # Add occasional artifacts
        # Eye blinks (if frontal channel)
        if ch < 3:  # First few channels get eye blinks
            blink_times = np.random.choice(n_samples, size=int(n_samples/sfreq/5), replace=False)  # Every 5 seconds average
            for blink_time in blink_times:
                if blink_time + int(0.2*sfreq) < n_samples:  # 200ms blink
                    blink_profile = np.exp(-np.linspace(0, 5, int(0.2*sfreq))**2) * 100e-6
        ...
        if n_channels * 0.3 < ch < n_channels * 0.7:  # Temporal channels
            muscle_times = np.random.choice(n_samples, size=int(n_samples/sfreq/10), replace=False)
            for muscle_time in muscle_times:
                if muscle_time + int(0.1*sfreq) < n_samples:  # 100ms muscle burst
                    ...
                    noise[muscle_time:muscle_time+len(muscle_profile)] += muscle_profile
...
def create_synthetic_raw(montage: str = "GSN-HydroCel-129", n_channels: int = 129, 
                        duration: float = 60.0, sfreq: float = 1000.0,
                        seed: Optional[int] = 42) -> mne.io.Raw:
...
def create_synthetic_events(paradigm: str, duration: float, sfreq: float,
                           seed: Optional[int] = 42) -> np.ndarray:
    ...
    if paradigm == "resting":
        ...
        event_times = np.arange(1, duration-0.5, 0.5) * sfreq  # Every 500ms starting at 1s
        n_events = len(event_times)
        
        # Create event IDs (80% standard, 20% deviant)
        event_ids = np.ones(n_events, dtype=int)  # Start with all standard
        deviant_indices = np.random.choice(n_events, size=int(0.2 * n_events), replace=False)
...
def save_synthetic_data_files(test_data_dir: Path, overwrite: bool = False) -> Dict[str, Path]:
    ...
    test_configs = [
        {"name": "resting_129ch", "montage": "GSN-HydroCel-129", "n_channels": 129, "paradigm": "resting"},
        {"name": "resting_128ch", "montage": "GSN-HydroCel-128", "n_channels": 128, "paradigm": "resting"},
        {"name": "chirp_129ch", "montage": "GSN-HydroCel-129", "n_channels": 129, "paradigm": "chirp"},
        {"name": "mmn_129ch", "montage": "GSN-HydroCel-129", "n_channels": 129, "paradigm": "mmn"},
        {"name": "resting_1020", "montage": "standard_1020", "n_channels": 32, "paradigm": "resting"},
        {"name": "bad_channels", "montage": "GSN-HydroCel-129", "n_channels": 129, "paradigm": "resting"},  # Will add many bad channels
    ]
    ...
    for config in test_configs:
        ...
        if overwrite or not events_file.exists():
            ...
            if len(events) > 0:
                mne.write_events(events_file, events, overwrite=overwrite, verbose=False)
...
def create_corrupted_data_samples(test_data_dir: Path) -> Dict[str, Path]:
    ...
    raw_bad_channels.info['bads'] = raw_bad_channels.ch_names[:100]  # Mark most channels as bad
