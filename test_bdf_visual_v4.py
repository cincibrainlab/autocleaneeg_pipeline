#!/usr/bin/env python3
"""
BDF Import Visual Diagnostic Report - Space-Optimized v4 (Academic Style)

Space-efficient academic-style validation report with optimized layouts:
- Multi-column metadata sections
- Side-by-side visualizations
- Compact tables with reduced padding
- Grid-based stat cards
- Optimized for both screen and print

Usage: uv run test_bdf_visual_v4.py [bdf_file] [montage_name] [output_dir]
"""
...
def generate_html_report(
    ...
    """Generate space-optimized professional HTML report."""
...
<body>
    <div class="container">
        ...
        <div class="verdict {verdict_class}">
            <div style="margin-bottom: 5px;">{verdict_icon} {verdict_title}</div>
            <div style="font-size: 11px; font-weight: normal;">
                Channel Match: {analysis['match_pct']:.1f}% ({len(analysis['matched'])}/{len(analysis['file_channels'])}) |
                Positioned: {analysis['n_positioned']}/{len(analysis['file_channels'])}
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div class="two-col-grid">
                <div class="col-section">
                    ...
                    <div class="info-row">
                        ...
                        <div class="info-value">{metadata['sampling_rate']}</div>
                    </div>
                    <div class="info-row">
                ...
                <div class="col-section">
                    <h3>Channel & Device Information</h3>
                    <div class="info-row">
                        <div class="info-label">Montage</div>
                        <div class="info-value">{montage_name}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Total Channels</div>
                        <div class="info-value">{metadata['n_channels']}</div>
                    </div>
                    <div class="info-row">
            ...
            {'<div class="info-box" style="margin-top: 10px;"><strong>Channel Types:</strong> ' + ', '.join([f'{k}: {v}' for k, v in metadata["channel_types"].items()]) + '</div>' if metadata['channel_types'] else ''}
        </div>

        <div class="section">
            <h2>Match Analysis & Position Quality</h2>
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="value">{analysis['mean_distance']:.4f}m</div>
                </div>
            </div>
            ...
            {f'''<div class="error-box">
                <strong>✗ DUPLICATE POSITIONS DETECTED!</strong> This usually indicates <strong>WRONG MONTAGE</strong> selected!
            </div>''' if analysis['duplicates'] else ''}

            {f'''<div class="warning-box">
                <strong>⚠ Position outliers detected:</strong> {', '.join(analysis['outliers'])}
            </div>''' if analysis['outliers'] else ''}
        </div>

        <div class="section">
            <h2>Electrode Position Visualizations</h2>
            <div class="image-grid">
                <div class="image-container">
                    <img src="{topomap_b64}" alt="2D Topomap">
                    <div style="font-size: 9px; margin-top: 5px; color: #666;">2D Electrode Positions</div>
                </div>
                <div class="image-container">
                    <img src="{stats_b64}" alt="Statistical Analysis">
                    <div style="font-size: 9px; margin-top: 5px; color: #666;">Statistical Analysis</div>
                </div>
            </div>
            <div class="image-container full">
                <img src="{plot3d_b64}" alt="3D Positions" style="max-width: 90%;">
                <div style="font-size: 9px; margin-top: 5px; color: #666;">3D Electrode Positions (Multiple Views)</div>
            </div>
            <div class="info-box">
                <strong>Legend:</strong> Green circles = Matched channels with positions | Orange squares = Unmatched channels
            </div>
        </div>

        <div class="section">
            <h2>Raw Channel Information</h2>
            <p style="color: #7f8c8d; margin-bottom: 12px; font-size: 10px;">
                Detailed channel-by-channel information extracted from the raw file before montage application.
            </p>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        ...
                        {f'<tr><td colspan="6" style="text-align: center; font-style: italic; padding: 8px;">... +{len(analysis["channel_data"]) - 30} more channels (showing first 30)</td></tr>' if len(analysis['channel_data']) > 30 else ''}
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="footer">
            <div>EEG Channel Validation Report v4.0 (Space-Optimized Academic Style)</div>
            <div style="margin-top: 5px;">Generated by AutoClean EEG Pipeline</div>
        </div>
    </div>
</body>
</html>"""
...
def main():
    ...
    console.print("\n[bold cyan]═══════════════════════════════════════════════════════════[/bold cyan]")
    console.print("[bold cyan]    BDF Visual Validation Report v4 (Space-Optimized)[/bold cyan]")
    ...
    html_file = output_dir / f"{Path(bdf_file).stem}_validation_report_v4.html"
